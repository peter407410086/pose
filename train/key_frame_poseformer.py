...
def calculate_mpjpe(x_hat):
    ...
    ground_truth = np.load('data/split_train_data_with_mirror_2/pkl/l_dribble_1_0.pkl', allow_pickle=True)
    ground_truth = calculate_angle(ground_truth)
    ground_truth = calculate_position(ground_truth) # [frames, 45]

    # 計算右手與右腳的y軸座標差距
    # rhand_index = jointIndex['rhand']
    # rfoot_index = jointIndex['rankle']
    # rhand_diff_y = np.abs(ground_truth[0, rhand_index+1] - ground_truth[0, rfoot_index+1])
    # print(f"groud_truth[0, rhand_index+1] = {ground_truth[0, rhand_index+1]:.2f}")
    # print(f"groud_truth[0, rfoot_index+1] = {ground_truth[0, rfoot_index+1]:.2f}")
    # print(f"rhand_diff_y: {rhand_diff_y*100:.2f}")

    # 印出ground_truth的每幀的左手關節座標
    lhand_index = jointIndex['lhand']
    # for i in range(frames):
    #     print(f"frame{i} = {x_hat[i, lhand_index:lhand_index+3]}")

    # Loop through all frames in the sequence
    for i in range(frames):
        # Calculate the MPJPE for the current frame
        # print(f"ground_truth[21:24] = {ground_truth.shape}")
        # print(f"x_hat[21:24] = {x_hat[21:24]}")
        mpjpe += np.mean(np.linalg.norm(ground_truth[i][21:24] - x_hat[i][21:24], axis=0))

    # Return the average MPJPE across all frames
    return mpjpe * 100 / frames

def calculate_hand_mpjpe(data):
    # hand,first_max_height,min_height,second_max_height,dribbling_frequency,bending_angle,duration
    # 1,0.6106176376342773,0.0765000581741333,0.6090077757835388,1.3636363636363638,5.742050787431325,22
    data_path = 'data/split_train_data/pkl/r_dribble_7_17.pkl'

    # 計算右手的座標與ground truth的差距
    ground_truth = np.load(data_path, allow_pickle=True)
    rhand_index = jointIndex['rhand']
    # print(ground_truth[0, rhand_index:rhand_index+3])
    rhand_coords = data[0, rhand_index:rhand_index+3]*100
    print(f"右手座標: {rhand_coords[0]:.2f}, {rhand_coords[1]:.2f}, {rhand_coords[2]:.2f}")

    # 計算兩者的右手座標差距
    rhand_diff = np.linalg.norm(data[0, rhand_index:rhand_index+3] - ground_truth[0][rhand_index:rhand_index+3], axis=0)
    
    # 計算兩者的右手y座標差距
    rhand_diff_y = np.abs(data[0, rhand_index+1] - ground_truth[0][rhand_index+1])
    print(f"rhand_diff_y: {rhand_diff_y*100:.2f}")
    print(f"rhand_diff: {rhand_diff.mean()*100:.2f}")
...
def weighted_mpjpe_loss_fn(x_hat, x, key_frames, weight=2.0):
    # print(f"x_hat.shape = {x_hat.shape}")
    # print(f"key_frames.shape = {key_frames.shape}")
    # print(f"key_frames = {key_frames[0]}")
    # 計算每個關節的位置誤差
    per_joint_position_error = torch.sqrt(torch.sum((x_hat - x)**2, dim=1))

    # 創建一個與 per_joint_position_error 相同形狀的權重陣列，並填充為 1
    weights = torch.ones_like(per_joint_position_error)

    # 將 key_frames 的權重設為指定的值
    for i in range(weights.shape[0]):
        weights[i, key_frames] = weight

    # 計算加權 MPJPE
    loss = torch.mean(weights * per_joint_position_error)
...
class MotionDataset(Dataset):
    def __init__(self, data_path, label_path, key_frame_path):
        """
        用於讀取運動數據集的類別。
...
class CVAE(nn.Module):
    def __init__(self, max_len=24, input_size=45, num_layers=4, hidden_size=512, num_heads=8, dropout=0.1, latent_size=256, label_size=7, in_chans=3, embed_dim_ratio=32, model_size=16, joints=15):
        super().__init__()
        embed_dim = embed_dim_ratio*in_chans # 96

        self.spatial_encoder = nn.Sequential(
            nn.Linear(in_chans, embed_dim_ratio), # [batch_size, max_len, 3] -> [batch_size, max_len, 32]
            PositionalEncoding(embed_dim_ratio, dropout=dropout, max_len=joints), # [batch_size, max_len, 32] -> [batch_size, max_len, 32]
            nn.TransformerEncoder( # [batch_size, max_len, 32] -> [batch_size, max_len, 32]
                nn.TransformerEncoderLayer(embed_dim_ratio, num_heads, hidden_size, dropout, batch_first=True),
                num_layers
            )
        )
        
        self.temporal_encoder = nn.Sequential(
            nn.Linear(embed_dim_ratio*joints, model_size), # [batch_size, max_len, 32] -> [batch_size, max_len, 16
        ...
        self.decoder = nn.Sequential(
            ...
            nn.Unflatten(1, (max_len, input_size)), # 45*max_len -> [batch_size, max_len, input_size]
            ...
            nn.Linear(model_size, input_size) # [batch_size, max_len, input_size] -> [batch_size, max_len, input_size]
        )

    def encode(self, x):
        # x: [batch_size, frames, 45]
        ...
        x = self.spatial_encoder(x) # [batch_size*frames, 32]

        # reshape x from (b*f, j, e) to (b, f, j, e)
        x = rearrange(x, '(b f) j e -> b f (j e)', b = b, f = f)#  [batch_size, frames, 480]

        # pass x through the temporal encoder
        x = self.temporal_encoder(x) # [batch_size, frames, 256]

        # split the encoded tensor into mu and logvar
        mu, logvar = torch.chunk(x, 2, dim=1) # [batch_size, latent_size], [batch_size, latent_size]
...
def train(data_path, label_path, max_epochs, batch_size, model_path):
    ...
    Returns:
        None
    """
    dataset = MotionDataset(data_path, label_path, key_frame_path)
    ...
    cvae = CVAE(label_size = dataset[0][1].shape[1], latent_size = latent_size, hidden_size=hidden_size)
    cvae = cvae.to('cuda')
    ...
    for epoch in range(max_epochs):
        cvae.train()
        train_loss = 0.0
        for i, (x, label, key_frame) in enumerate(tqdm(train_loader, desc="training")):
            ...
            # n = int(label[i][0][6])
            # 印出x第n幀的值
            # print(f"x[0][{n-1}][0:3] = {x[0][n-1][0:3]}")
            # print(f"x[0][{n}][0:3] = {x[0][n][0:3]}")
            x = x.float().to(device)
            ...
            loss = weighted_mpjpe_loss_fn(x_hat, x, key_frame, weight=2.0)
...
def generate(num_samples, label_path, model_path):
    ...
    cvae = CVAE(label_size = label_size, latent_size = latent_size, hidden_size=hidden_size)
    ...
    # 畫出 MPJPE 的結果(固定的label，算每次的mpjpe)
    plt.figure()
    plt.boxplot(mpjpe_results)
...
if __name__ == "__main__":
    ...
    generate(num_samples=20, label_path=label_path, model_path=model_path)
